...
-- Production-Quality Initialization with Real Organizational Organizational Data
-- ============================================================================
-- Purpose: Populate initial data for grant management system with authentic
-- Organizational content and information information, and program descriptions
-- Created: 2024
-- ============================================================================
...
INSERT INTO users (email, name, role, is_active, created_at, updated_at) VALUES
  ('admin@grantengine.org', 'System Administrator', 'admin', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
  ('levar.robinson@grantengine.org', 'Levar Robinson', 'executive_director', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
  ('grants@example.org', 'Grants Team', 'grant_writer', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
  ('programs@grantengine.org', 'Program Director', 'program_director', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
  ('compliance@grantengine.org', 'Compliance Officer', 'compliance_officer', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
...
INSERT INTO boilerplate_categories (category_name, description, sort_order, is_active, created_at, updated_at) VALUES
  ...
  ('Program-Specific Language', 'Detailed descriptions of organizational programs, methods, evaluation, and outcomes', 3, true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
...
INSERT INTO tags (name, tag_type, description, is_active, created_at) VALUES
  -- Program tags
  ('Fatherhood Education', 'program', 'organizational core fatherhood programming', true, CURRENT_TIMESTAMP),
...
'The organization is based in East Baton Rouge Parish, Louisiana, and serves a defined geographic service area encompassing zip codes 70802, 70805, 70806, 70807, and 70812. These service areas represent neighborhoods with significant concentrations of single-parent households, economic challenges, and families at risk of child welfare involvement. The organization maintains office facilities and program delivery capacity to serve the target population within this geographic scope.',
...
FROM boilerplate_categories bc, users u
WHERE bc.category_name = 'Organizational Narrative' AND u.email = 'admin@grantengine.org';
...
'The East Baton Rouge Parish community presents significant challenges that drive the need for The organization''s services. The parish encompasses a total population of 453,022 residents, with 101,169 children (22.3% of population). This community faces substantial socioeconomic disparities: 27% of children live below the poverty line, compared to the state average of 26% and national average of 20%. Single-parent households comprise 44.67% of family structures, with female-headed households representing the majority of single-parent arrangements. These conditions create cumulative risk factors for children: economic instability, reduced parental capacity, increased child maltreatment risk, and educational disruption. The organizational service area zip codes (70802, 70805, 70806, 70807, 70812) experience even more severe concentrations of these risk factors. Research demonstrates that father engagement is protective against these risks and promotes positive child outcomes. The organization''s location-based service strategy targets the highest-need neighborhoods where families are most vulnerable and where strategic fatherhood intervention can yield substantial impact.',
...
FROM boilerplate_categories bc, users u
WHERE bc.category_name = 'Program-Specific Language' AND u.email = 'admin@grantengine.org';
...
WITH admin_user AS (
  SELECT id FROM users WHERE email = 'admin@grantengine.org'
)
...
WITH current_user AS (
  SELECT id FROM users WHERE email = 'grants@example.org'
)
...
--
-- All content is production-quality and incorporates real organizational organizational data.
